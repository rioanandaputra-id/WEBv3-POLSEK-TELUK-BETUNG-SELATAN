        ...
        <div class="col">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <div class="row">
                        <div class="col">
                            <h1 class="h5 text-gray-800"><strong>MENU INFORMASI UBAH BURONAN</strong></h1>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            <form id="test" enctype="multipart/form-data" method="POST" onsubmit="return false">
                                <?php foreach ($buronan as $record) : ?>
                                    <div class="row mb-2">
                                        <div class="col-sm-2">
                                            <label><strong>Foto: <i class="text-danger">*</i></strong></label>
                                            <?php $foto = ($record->TIPE_FOTO == 'url') ? $record->FOTO : base_url('assets/upload/buronan/') . $record->FOTO; ?>
                                            <img id="image-preview" src="<?= $foto; ?>" width="140" height="120" onclick="show_modal()">
                                            <input type="hidden" name="ID" id="ID" value="<?= $record->ID_BURONAN; ?>">
                                        ...
                                        <div class="col-sm-10">
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="col">
                                                    ...
                                                    <input type="text" class="form-control" name="KASUS" value="<?= $record->KASUS; ?>">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    function show_modal() {
        $('#modal-title').text("Unggah Foto Buronan");
    ...
    function btn_unggah() {
        if ($('#TIPE_FOTO').val() === 'unggah') {
            deleteImage('<?= base_url('assets/upload/buronan/') ?>' + $('#FOTO').val());
        }
        var formData = new FormData($("#modal-upload")[0]);
        $.ajax({
            url: "<?php echo site_url('admin/buronan_image_upload') ?>",
            cache: false,
            contentType: false,
            processData: false,
            data: formData,
            ...
            success: function(res) {
                if (res.type == 'url') {
                    ...
                    $('#image-preview').attr('src', '<?= base_url('assets/upload/buronan/') ?>' + res.url);
                }
    ...
    function form_submit() {
        ...
        $.ajax({
            url: "<?= base_url('admin/buronan_update/'); ?>",
            type: 'POST',
            data: formData,
            cache: false,
            contentType: false,
            ...
            success: function(data) {
                if (data.status == 'false') {
                    ...
                    Swal.fire(
                        ...
                        window.location.href = "<?= base_url('admin/informasi/buronan/'); ?>";
                    });
                }
            },
